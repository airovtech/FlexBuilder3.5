package smartWork.custormObj
{
	...
	[Event(name="panelClose", type="smartWork.custormObj.event.PanelEvent")]
	public class MovePanel extends Panel {
		...
		[Bindable] 
		public var enableMinimize:Boolean = false;
		[Bindable] 
		public var enableIncrease:Boolean = false;
		[Bindable] 
		public var enableClose:Boolean = true;
		[Bindable] 
		public var pTitleBarHeight:Number = 0;
		...
		private var normalMinButton:Button	= new Button();
		...
		override protected function createChildren():void {
			...
			if (showControls) {
				if(enableMinimize){
					...
					this.pTitleBar.addChild(this.normalMinButton);
				}
				if(enableIncrease){
		...
		public function positionChildren():void {
			if (showControls) {
				this.normalMinButton.buttonMode    = true;
				this.normalMinButton.useHandCursor = true;
				this.normalMinButton.x = this.unscaledWidth - this.normalMinButton.width - 40;
				this.normalMinButton.y = 8;
		...
		private function doResize(event:ResizeEvent):void{
			MovePanel(event.target).positionChildren(); // this.positionChildren(); 와 같다.
        }
