...
package com.maninsoft.smart.modeler.xpdl.syntax
{
	 ...
	 */
	public class NoneInLinkError extends ActivityProblem {
		...
		/** Constructor */
		public function NoneInLinkError(activity: Activity) {
			super(activity);

			level = LEVEL_ERROR;
			label = resourceManager.getString("ProcessEditorMessages", "PEP008L");
			message = resourceManager.getString("ProcessEditorMessages", "PEP008M");
			description = resourceManager.getString("ProcessEditorMessages", "PEP008D");
		}
		
		public static  function checkIt(buff: ArrayCollection, activity: Activity): void {
			addCount(NoneInLinkError);
			
			if (!(activity is StartEvent) && !activity.hasIncoming) {
				buff.addItem(new NoneInLinkError(activity));
			}
		}
